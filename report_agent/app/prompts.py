...
- 분석 결과는 최종 보고서의 네 구성요소에만 쓰입니다: 요약, 위험도 분포 및 평가 추세, 주요 고위험 항목, 결론.

분석 범위:
- 포함: 전체 평가 건수, 고위험/중대위험 비율, 위험도 분포, 주요 고위험 항목, 미조치 고위험 항목, 조치 완료율, 승인 완료율, 일자/주차별 평가 추세
...
- 본사보고용 추세보고서의 표현 방식은 참고하되, 최종 산출물은 위험성평가보고서로 작성합니다.

보고서 구성은 반드시 아래 4개만 사용하세요:
1. 요약
   - KPI 요약과 사업장 전체 평가 결과 요약을 하나로 합친 섹션입니다.
   - 전체 평가 건수, 고위험/중대위험 건수 및 비율, 조치 완료율, 승인 완료율을 포함하세요.
2. 위험도 분포 및 평가 추세
   - 위험도 분포와 일자/주차별 평가 추세만 다룹니다.
...
- 추세 해석은 "위험도 분포 및 평가 추세" 섹션에서만 1회 구체적으로 작성하고, 요약/결론에서는 같은 문장을 반복하지 마세요.
...
수치 기준:
- 주요 수치는 aggregated_data.kpi, trend, risk_distribution과 정확히 일치해야 합니다.
- 검토 결과가 제공되면 revision_instructions를 반영해 다시 작성하세요.
"""
...
검토 기준:
- 보고서 구성요소가 정확히 다음 4개인지 확인하세요: 요약, 위험도 분포 및 평가 추세, 주요 고위험 항목, 결론.
- KPI 수치가 aggregated_data.kpi와 일치하는지 확인하세요.
- 위험도 분포와 추세 설명이 aggregated_data.risk_distribution, trend와 모순되지 않는지 확인하세요.
