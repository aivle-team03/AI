...
from app.management.prompts import (
    MANAGEMENT_DATA_CORRECTION_PROMPT,
    MANAGEMENT_DATA_CORRECTION_REVIEW_PROMPT,
)
...
async def management_data_correction_agent(
    rows,
    ...
    return await llm.ainvoke(
        [
            SystemMessage(content=MANAGEMENT_DATA_CORRECTION_PROMPT),
            HumanMessage(content=json.dumps(payload, ensure_ascii=False, indent=2)),
        ]
    )


async def management_data_correction_review_agent(original_rows, correction_result):
    ...
    return await llm.ainvoke(
        [
            SystemMessage(content=MANAGEMENT_DATA_CORRECTION_REVIEW_PROMPT),
            HumanMessage(content=json.dumps(payload, ensure_ascii=False, indent=2)),
        ]
    )
