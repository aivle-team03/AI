...
async def verify_action_endpoint(
    ...
    try:
        ...
        다른 카테고리나 위험 요소는 절대 고려하지 말고, 오직 지정된 **[{category_name}]** 카테고리 하나만 보고 안전 여부를 판단하세요.

        [검사 대상 카테고리]: [{category_name}]
        ...
        [출력 규칙]
        1. 지정 카테고리가 안전하게 조치되었으면 is_resolved를 true, 미조치 상태면 false로 지정하세요.
        2. result_text는 조치 완료 시 "위험요소 해소", 미해소 시 "위험요소 미해소"로 작성하세요.
        3. analysis_summary에는 **[{category_name}]** 관점에서 판단한 근거를 한 문장의 한국어로 작성하세요.
